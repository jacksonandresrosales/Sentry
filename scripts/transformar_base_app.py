"""Mini aplicación local para cargar una base y generar su DB_delete.xlsx."""
...
class DeleteBaseApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("DB delete — Bases de llamadas")
    ...
    def build_ui(self):
        ...
        ttk.Label(frame, text="DB delete", font=("Segoe UI", 20, "bold")).grid(
            row=0, column=0, columnspan=2, sticky="w")
        ttk.Label(frame, text="Carga una base y obtén el Excel con la estructura del modelo NO.").grid(
            row=1, column=0, columnspan=2, sticky="w", pady=(4, 16))
        ttk.Label(frame, text="Base de entrada (.csv o .xlsx)").grid(row=2, column=0, sticky="w")
        ...
        select.grid(row=3, column=1, padx=(10, 0), pady=(4, 12))
        ttk.Label(frame, text="Carpeta de resultados").grid(row=4, column=0, sticky="w")
        ...
        folder.grid(row=5, column=1, padx=(10, 0), pady=(4, 10))
        options = ttk.Frame(frame)
        options.grid(row=6, column=0, columnspan=2, sticky="ew")
        ...
        self.process_button.grid(row=8, column=0, columnspan=2, sticky="ew")
        self.progress = ttk.Progressbar(frame, mode="indeterminate")
        self.progress.grid(row=9, column=0, columnspan=2, sticky="ew", pady=(12, 6))
        ...
        ttk.Label(frame, textvariable=self.saved_path, wraplength=680).grid(
            row=11, column=0, columnspan=2, sticky="w", pady=(5, 8))
        actions = ttk.Frame(frame)
        actions.grid(row=12, column=0, columnspan=2, sticky="w")
        ...
        self.controls = [select, folder, type_box, number_entry, self.process_button]
    ...
    def refresh_output_name(self):
        if not self.input_path.get():
            return
        try:
            number = self.base_number.get().strip() or None
    ...
    def start_conversion(self):
        ...
        try:
            value = self.base_number.get().strip()
        ...
        threading.Thread(target=self.worker,
                         args=(source, folder, base_type, base_number), daemon=True).start()

    def worker(self, source, folder, base_type, base_number):
        try:
            result = convert_files([source], base_type=base_type,
                                   base_number=base_number, output_folder=folder)
