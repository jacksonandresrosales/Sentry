        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            try:
                ...
                with patch("app.services.audio_analysis.transcribe", return_value=TRANSCRIPT), \
                    ...
                    self.assertTrue(window.analyze_button.text().startswith("Detener · 0/"))
                    self.assertEqual(window.analyze_button.progress_ratio, 0)
                    self.assertFalse(window.play_button.isEnabled())
                    self.assertFalse(window.jump_button.isEnabled())
                    self.assertFalse(window.original_button.isEnabled())
                    self.assertFalse(window.reviewed_button.isEnabled())
                    self.assertFalse(window.timeline.isEnabled())
                    self.wait(window)
                self.assertIsNone(window.analyze_button.progress_ratio)
                self.assertEqual(window.analyze_button.text(), "Analizar")
                self.assertTrue(window.play_button.isEnabled())
                self.assertTrue(window.original_button.isEnabled())
                self.assertTrue(window.reviewed_button.isEnabled())
